...
from source.dialog.ExportDialog import ExportDialog
from source.dialog.BroadcastDialog import BroadcastDialog
from source.dialog.CloneDialog import CloneDialog
...
class MainMenu:
    def __init__(self, telegram):
        ...
        self.broadcast_dialog = BroadcastDialog()
        self.clone_dialog = CloneDialog()
    ...
    def _init_menu_options(self):
        return [
            ...
            {"name": "Clone Channel (Wizard)", "value": "10", "handler": self.clone_channel},
            {"name": "Export Chat History", "value": "8", "handler": self.export_chat},
            {"name": "Broadcast Message", "value": "9", "handler": self.broadcast_message},
            {"name": "Statistics", "value": "11", "handler": self.show_statistics},
    ...
    async def find_user(self):
        result = await self.find_user_dialog.get_config()
        if result and result[0]:
            await self.telegram.find_user(result)

    async def export_chat(self):
        config = await self.export_dialog.get_config()
        if config:
            await self.telegram.export_chat_history(config)

    async def broadcast_message(self):
        config = await self.broadcast_dialog.get_config()
        if config:
            await self.telegram.broadcast_message(config)

    async def clone_channel(self):
        config = await self.clone_dialog.get_config()
        if config:
            await self.telegram.clone_channel(config)

    async def show_statistics(self):
        await self.telegram.show_statistics()
